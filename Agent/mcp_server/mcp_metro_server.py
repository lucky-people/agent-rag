...
# -*- coding: utf-8 -*-
"""
文件名: mcp_metro_server.py
作者: 智租顾问 (Zhizu Advisor)
描述: 交通出行MCP服务器，基于 rental 库的 metro_station 表（可通过 poi_data 查地标最近地铁站）
"""
import os
import sys
...
# 地铁服务类
class MetroService:  # 定义地铁服务类，封装数据库操作逻辑
    def __init__(self):
        # 连接数据库
    ...
    def execute_query(self, sql: str) -> str:
        try:
            ...
            return json.dumps({"status": "success", "data": results} if results else {"status": "no_data",
                                                                                      "message": "未找到相关地铁信息，请确认站名或位置。"},
                              cls=DateEncoder, ensure_ascii=False)
        except Exception as e:
            logger.error(f"地铁查询错误: {str(e)}")
            # 返回错误JSON响应
            return json.dumps({"status": "error", "message": str(e)}, ensure_ascii=False)


# 创建地铁MCP服务器
def create_metro_mcp_server():
    # 创建FastMCP实例
    metro_mcp = FastMCP(name="MetroTools",
                        instructions="交通出行工具，基于 metro_station 表查询地铁站与线路，可基于 poi_data 表查询地标最近地铁站。",
                        log_level="ERROR",
                        host="127.0.0.1", port=8006)

    # 实例化地铁服务对象（若数据库不可用，给出清晰提示，避免端口未监听导致A2A侧连接失败）
    try:
    ...
    @metro_mcp.tool(
        name="query_metro",
        description="查询地铁信息，输入 SQL，如 'SELECT * FROM metro_station WHERE name = \"郑州东站\"'"
    )
    def query_metro(sql: str) -> str:
        logger.info(f"执行地铁查询: {sql}")
        return service.execute_query(sql)

    # 打印服务器信息
    logger.info("=== 地铁MCP服务器信息 ===")
    logger.info(f"名称: {metro_mcp.name}")
    logger.info(f"描述: {metro_mcp.instructions}")

    # 运行服务器
    try:
        logger.info("地铁MCP服务器已启动，请访问 http://127.0.0.1:8006/mcp")
        metro_mcp.run(transport="streamable-http")
    except Exception as e:
        logger.error(f"地铁MCP服务器启动失败: {e}")


if __name__ == '__main__':
    create_metro_mcp_server()
