...
# -*- coding: utf-8 -*-
"""
文件名: mcp_poi_server.py
作者: 智租顾问 (Zhizu Advisor)
描述: 周边探索MCP服务器，基于 rental 库的 poi_data 表
"""
import os
import sys
...
# POI服务类
class PoiService:  # 定义POI服务类，封装数据库操作逻辑
    def __init__(self):
        # 连接数据库
    ...
    def execute_query(self, sql: str) -> str:
        try:
            ...
            return json.dumps({"status": "success", "data": results} if results else {"status": "no_data",
                                                                                      "message": "未找到相关地点，请确认查询条件。"},
                              cls=DateEncoder, ensure_ascii=False)
        except Exception as e:
            logger.error(f"POI查询错误: {str(e)}")
            # 返回错误JSON响应
            return json.dumps({"status": "error", "message": str(e)}, ensure_ascii=False)


# 创建POI MCP服务器
def create_poi_mcp_server():
    # 创建FastMCP实例
    poi_mcp = FastMCP(name="PoiTools",
                      instructions="周边探索工具，基于 poi_data 表，查询郑州景点、美食、公园、住宿等POI。",
                      log_level="ERROR",
                      host="127.0.0.1", port=8005)

    # 实例化POI服务对象（若数据库不可用，给出清晰提示，避免端口未监听导致A2A侧连接失败）
    try:
        service = PoiService()
    except Exception as e:
    ...
    @poi_mcp.tool(
        name="query_poi",
        description="查询POI数据，输入 SQL，如 'SELECT * FROM poi_data WHERE type = \"旅游景点\" AND nearest_metro_line LIKE \"%1号线%\"'"
    )
    def query_poi(sql: str) -> str:
        logger.info(f"执行POI查询: {sql}")
        return service.execute_query(sql)

    # 打印服务器信息
    logger.info("=== POI MCP服务器信息 ===")
    logger.info(f"名称: {poi_mcp.name}")
    logger.info(f"描述: {poi_mcp.instructions}")

    # 运行服务器
    try:
        logger.info("POI MCP服务器已启动，请访问 http://127.0.0.1:8005/mcp")
        poi_mcp.run(transport="streamable-http")
    except Exception as e:
        logger.error(f"POI MCP服务器启动失败: {e}")


if __name__ == '__main__':
    create_poi_mcp_server()
