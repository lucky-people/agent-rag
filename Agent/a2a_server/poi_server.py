...
# 周边探索查询服务器类
class PoiQueryServer(A2AServer):
    def __init__(self):
    ...
    def generate_sql_query(self, conversation: str) -> dict:
        try:
            ...
            return {"status": "input_required", "message": "查询无效，请提供周边探索条件（如地点、类型、线路）。"}  # 返回追问JSON
        except Exception as e:
    ...
    def handle_task(self, task):
        ...
        try:
            ...
            for attempt in range(1, max_attempts + 1):
                ...
                poi_result = asyncio.run(get_poi(sql_query))

                # 4 格式化结果
                response = json.loads(poi_result) if isinstance(poi_result, str) else poi_result
                ...
                if response.get("status") == "success":
                    ...
                    if attempt < max_attempts:
                        ...
                        break
                    task.status = TaskStatus(state=TaskState.INPUT_REQUIRED,
                                             message={"role": "agent", "content": {"text": "未找到符合条件的周边地点，已尝试放宽条件仍无结果，请换个说法再试。"}})
                    return task
                else:
            ...
            if response_text is None:
                response_text = "未找到符合条件的周边地点，已尝试放宽条件仍无结果，请换个说法再试。"
            task.status = TaskStatus(state=TaskState.INPUT_REQUIRED,
                                     message={"role": "agent", "content": {"text": response_text}})
            return task
