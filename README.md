...
> 以下数据均来自**本仓库实际运行**的评估脚本（`实验脚本/`），非演示值；所有实验在同一台机器（CPU 推理模式）下复跑产出，覆盖检索 → 路由 → 生成 → 缓存四层。
...
对比 **纯 BM25 / 纯稠密向量 / 混合检索 / 混合 + Reranker** 四种策略（30 道租房法律题，10 大场景），实测数据：

| 策略 | Recall@5 | MRR | 平均耗时 | 命中 |
|------|---------|-----|---------|------|
| A. 纯 BM25 | 0.7333 | 0.6167 | 38.0 ms | 22/30 |
| B. 纯稠密向量（BGE-M3） | **0.8333** | 0.6178 | 582.0 ms | 25/30 |
| C. 混合检索（BM25+稠密+稀疏） | 0.7667 | **0.6583** | 491.6 ms | 23/30 |
| D. 混合 + BGE Reranker | 0.8000 | 0.5228 | 37103.5 ms | 24/30 |

<img src="实验脚本/results/rag_ablation_chart.png" width="820"/>

> **结果解读（面试亮点）**：
> - 纯稠密向量 Recall@5 最高（0.833），验证 BGE-M3 对**语义变体**（"扣押金合法吗" vs 法条表述）的召回能力；
> - 混合无 Rerank 的 **MRR 最优（0.6583）**——三路融合把"相关文档排名"质量拉高，说明混合策略的价值更多体现在排序而非单纯召回；
> - Reranker 在 CPU 上单题耗时 37s（生产环境 GPU 下为百毫秒级），且 Recall@5/MRR 并不占优——因为它优化的是 **Top-K 内的排序精度而非召回集合**，此项指标需结合最终答案质量评估（见实验四）；
> - 消融实验揭示的关键事实：**纯 BM25 对口语化法律问题召回率最低（22/30）**，直接论证了引入向量检索的必要性。

### 实验三：三链路响应时长对比（`compare_rag_redis_mysql.py`）
...
### 实验四：Agentic RAG vs 朴素 RAG 端到端对比（Self-RAG 反思价值量化）
...
python 实验脚本/run_rag_ablation.py      # RAG 消融
