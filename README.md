...
对比 **纯 BM25 / 纯稠密向量 / 混合检索 / 混合 + Reranker** 四种策略（Recall@5、MRR、平均耗时），验证混合检索 + 重排的增益：
...
| 链路 | 路径 | 平均耗时 |
|------|------|---------|
| A. MySQL（BM25 命中 FAQ） | jpkb 表直答 | ~6ms |
| B. Redis 缓存 | 命中 `rag_answer:*` 秒回 | 远快于全链路 |
| C. RAG 全链路 | BM25 + 向量 + Rerank + LLM 生成 | 完整生成（1s 级） |
