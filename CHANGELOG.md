...
### 修复
- **单元测试在无密钥环境无法运行**：4 个 A2A server 原先在 import 阶段就构造 LLM 客户端，缺 API Key 时直接抛 `openai.OpenAIError`，导致全新 clone / GitHub CI 跑 `unittest` 必然失败；现改为初始化失败降级为 `llm = None` + 告警，调用侧原有的 try/except 与单域回退照常生效（有密钥时行为不变）
